...
function AccordionItem(props:AccordionItem) {
  ...
  return (
    <div className='overflow-hidden last:border-b-0'>
      <div className={`group flex flex-row justify-between items-center ${isClose ? 'border-b-0' : 'border-b'}`}>
        <h4 className="w-full font-bold font-heading uppercase px-6">{props.title}</h4>
      ...
      <div className={`overflow-hidden ${isClose ? 'h-0' : 'h-fit'} transition duration-300`}>
        <p className='p-6 mt-2 mb-4 pr-20 md:pr-32'>{props.content}</p>
      </div>
    </div>
  )
}
