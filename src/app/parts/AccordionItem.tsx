    ...
    <div className='overflow-hidden last:border-b-0'>
      <div className={`flex flex-row justify-between items-center ${isClose ? 'border-b-0' : 'border-b-2'}`}>
        ...
        <div onClick={HandleAccordionClick} className='h-full p-6 cursor-pointer hover:bg-slate-100 *:hover:stroke-slate-600 transition-all duration-300'>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-6 h-6 ${isClose ? 'rotate-0 stroke-slate-400/60' : 'rotate-180 stroke-slate-800'} transition duration-300`}><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
        </div>
      </div>
