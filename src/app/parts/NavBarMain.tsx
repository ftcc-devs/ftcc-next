...
function NavBarMain() {
  ...
  return (
    <div id='navbar' className='flex flex-row justify-between items-center md:items-end w-full z-20 px-6 lg:px-12 pt-6 pb-5 lg:pt-18 lg:pb-18 bg-white'>
      ...
      <div className={`flex font-heading text-sm font-semibold uppercase tracking-wide ${menuToggle ? 'flex-row lg:flex lg-justify-start gap-0 items-center' : 'flex-col fixed justify-start items-end lg:items-end gap-6 w-full max-w-[220px] top-0 right-0 h-screen z-50 bg-indigo-950 px-10 py-12'}`}>
        <div id="nav-toggle" onClick={() => {setMenuToggle(!menuToggle)}} className={`w-12 h-8 ${menuToggle ? 'flex lg:hidden' : ''}`}>
          <Image
        ...
        <div id="nav-menu" className={`flex w-full ${menuToggle ? 'flex-col lg:flex-row justify-between items-center gap-12' : 'flex-col'}`}>
          <ul className={`lg:flex flex-row tracking-wider font-heading 
